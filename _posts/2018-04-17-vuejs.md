...
可以将getter与setter看作类似于进行相反操作。getter通过设置原数据来改变计算属性的值，而setter是通过设置计算属性，同步更新原数据的值。
```
computed:{
  priceIndex:{
    get:function(){
...
```
以上代码为priceIndex设置同时设置了setter和getter，所以当改变price的值时，princeIndex的值会改变(getter)；当改变priceIndex的值时，price的值也会改变(setter)。

### 监听/侦听属性-watch
当数据发生变化时骂我们可以利用**监听器**来响应数据的变化。即`watch`会实施监听数据变化并改变自身的值。  
